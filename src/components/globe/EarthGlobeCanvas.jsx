...
// Cloud infrastructure layer — server nodes hovering above the globe.
const INFRA_NODES = [
...
function infraPosition(lat, lng, altitude) {
  return latLonToVector3(lat, lng, EARTH_RADIUS + altitude)
}

function CloudInfrastructureLayer({ animated }) {
  ...
  // Build connection lines geometry. On premium tiers we emit discrete segments
  // (paired vertices) for a denser, smoother arc that supports additive glow.
  const lineGeometry = useMemo(() => {
    const points = []
    for (const [a, b] of INFRA_CONNECTIONS) {
      const pa = nodePositions[a]
      const pb = nodePositions[b]
      // Arc midpoint lifted slightly for curvature
...
const tex = (file) => `${LOCAL_TEXTURES}/${file}`

/** Bundled planet textures — same-origin only (no CDN / CSP external fetches). */
const OFFLINE_EARTH = {
...
// Art-directed key light. The Earth is fixed in world space while the camera
// orbits, so this fixed direction gives a stable, premium highlight and drives
// the night-side city-lights terminator.
const KEY_LIGHT_POSITION = [5.2, 2.8, 5.6]
const KEY_LIGHT_DIRECTION = new THREE.Vector3(...KEY_LIGHT_POSITION).normalize()

/** Earth keeps a lifted fill level so the full sphere stays readable (no harsh terminator). */

/**
 * Pick a quality tier from the device's actual capabilities so the globe runs
 * smoothly everywhere: GPU max texture size, CPU cores, device memory, mobile
 * heuristics, and the user's data-saver preference all factor in.
 */
function detectQualityTier() {
  ...
  try {
    ...
    // Respect data-saver and very constrained devices outright.
...
const QUALITY_PRESETS = {
  ...
  medium: {
    ...
    specular: [OFFLINE_EARTH.specular],
    night: [OFFLINE_EARTH.night],
    // Normal mapping is the priciest per-fragment cost; skip it on medium.
  ...
  low: {
    ...
    anisotropy: 8,
    starCount: 700,
    // Cheapest path: unlit base map, no extra layers, no scene lights.
    lit: false,
    day: [OFFLINE_EARTH.day],
    clouds: [],
    normal: [],
...
const ATMOS_FRAG = `
  ...
  void main() {
    ...
    // Brighten the limb facing the key light so the halo reads as scattered
    // light rather than a flat neon ring.
...
// Night city-lights: emissive on the side facing away from the key light, faded
// toward the limb so it never bleeds past the silhouette. (Synthesized from the
// premium-opening-globe night-lights shader.)
...
function configureTexture(texture, { srgb = false, anisotropy = 8 } = {}) {
...
/** Best-effort optional texture — resolves to null instead of throwing. */
function loadOptionalTexture(urls, options) {
...
/** Slowly drifting, sun-shaded cloud canopy floating just above the surface. */
function CloudLayer({ cloudMap, segments }) {
...
function NightLightsLayer({ nightMap }) {
  const uniforms = useMemo(
    () => ({
...
/** Distinctive, taller pulsing pin marking the corporate home base (NYC). */
function HomeBaseMarker({ home }) {
  const ringRef = useRef(null)
  const position = useMemo(
    () => latLonToVector3(home.lat, home.lng, EARTH_RADIUS + 0.02),
    [home.lat, home.lng],
  )
  // Orient the pin so it stands up off the surface (points away from center).
  ...
  return (
    <group position={position} quaternion={quaternion} renderOrder={20}>
      ...
      {/* Pulsing halo flush to the surface */}
...
function TexturedEarth({
  ...
  useEffect(() => {
    ...
    // Required day texture — failure here drops to the flat sphere fallback.
    loadFirstAvailable(quality.day, { srgb: true, anisotropy })
      .then((day) => {
        if (cancelled) return
        setTextures({ day, clouds: null, normal: null, specular: null, night: null })
        // The surface map is ready — signal so any preloader can dismiss while the
        // crossfade brings the textured Earth in.
        onReady?.()

        // Optional premium maps stream in afterward, gated per tier so low-end
        // devices never pay the bandwidth or per-fragment cost.
        void Promise.all([
  ...
  useFrame((_, delta) => {
    ...
    // Crossfade the textured surface in over the base sphere once it loads, so
    // the globe materializes smoothly instead of popping in on a single frame.
  ...
  return (
    <group ref={groupRef}>
      {/* Immediate opaque base so the globe silhouette is present from the very
          first frame (no empty void → sudden planet). The textured surface, a
          hair larger and non-depth-writing, crossfades on top of it. */}
      <mesh>
...
function GlobeScene({
  ...
  useEffect(() => {
    // Transparent scene — the globe floats directly inside the glass UI card.
  ...
  return (
    <>
      {/* Lighting rig — only mounted for the lit (medium/high) tiers; the low
          tier renders an unlit base map and needs no lights. */}
      {quality.lit ? (
        <>
      ...
      <Stars radius={180} depth={80} count={stars} factor={2.9} saturation={0.14} fade speed={0.2} />
      <OrbitControls
      ...
      <SceneErrorBoundary fallback={fallback}>
        {failed ? (
          ...
          <TexturedEarth
            ...
            onError={() => {
              setFailed(true)
              // Never leave the preloader hanging if textures fail.
              onReady?.()
            }}
            onReady={onReady}
          />
      ...
      {/* Subtler, realistic atmosphere — dialed back so the Earth doesn't glow like neon. */}
...
export function GlobeStaticFallback({ className }) {
  return (
    <div
...
export function EarthGlobeCanvas({
  ...
  // When focusing on a location, position the camera along that point's surface
  // normal so it faces the camera directly, and zero out the base yaw so the
  // globe's local frame matches world space (markers stay put).
  ...
  // No WebGL → we render the static fallback; let any preloader dismiss.
  useEffect(() => {
    if (!webglOk) onReady?.()
  }, [webglOk, onReady])
